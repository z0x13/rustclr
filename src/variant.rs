...
pub fn create_safe_array_buffer(data: &[u8]) -> Result<SafeArrayWrapper> {
    ...
    unsafe {
        ...
        if let Err(err) = SafeArrayAccessData(sa, &mut p_data) {
            let _ = SafeArrayDestroy(sa);
            return Err(ClrError::ApiError("SafeArrayAccessData", err.code().0));
        }
        ...
        if let Err(err) = SafeArrayUnaccessData(sa) {
            let _ = SafeArrayDestroy(sa);
            return Err(ClrError::ApiError("SafeArrayUnaccessData", err.code().0));
        }
...
pub fn create_string_array_variant(strings: Vec<String>) -> Result<VARIANT> {
    ...
    let variant = unsafe { InitVariantFromStringArray(&pcwstr_vec) }
        .map_err(|err| ClrError::ApiError("InitVariantFromStringArray", err.code().0))?;
